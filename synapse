...
--local uiss = window:MakeTab({Name = "Other GUIs"})
...
main:AddToggle({Name = "AutoGaurd/Defend", default = false, Callback = function(v)
    if v then
        game.StarterGui:SetCore("SendNotification", {Title = "AutoGaurd/Defend", Text = "Press X to go to the nearest ball", Duration = 5, Button1 = "Okay"})
    end
    _G.Autogaurd = v
end})
main:AddToggle({Name = "Infinite Jump", Default = false, Callback = function(v)
    _G.infinjump = v
end})
...
g:AddSlider({Name = "WalkSpeed", Min = 16, Max = 19, Default = 16, Color = Color3.fromRGB(80, 80, 255), Increment = 0.1, Callback = function(v)
    _G.WS = v
end})
g:AddSlider({Name = "HipHeight", Min = 0, Max = 9, Default = 0, Color = Color3.fromRGB(80, 80, 255), Increment = 0.1, Callback = function(v)
    _G.HH = v
end})
s:AddToggle({Name = "Silent Mode", Default = false, Callback = function(v)
    _G.Silent = v
end})
s:AddToggle({Name = "Auto Power", Default = false, Callback = function(v)
    _G.AutoPower = v
end})
...
function stepped() pcall(function()
    ...
    if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and _G.AutoPower then
        local pwr = power()
        local dist, goal = setup()
        local root = plr.Character.HumanoidRootPart
        ...
        elseif root and not hasBall and _G.AutoPower then
            root.Transparency = 1
        elseif root and not _G.AutoPower then
            root.Transparency = 1
        end
    end
    ...
    if _G.HH ~= 0 and plr.Character:WaitForChild("Humanoid").HipHeight then
        plr.Character:WaitForChild("Humanoid").HipHeight = _G.HH
    end
    ...
    for i,v in pairs(game.Players:GetPlayers()) do
        if (v.Name ~= plr.Name and v.Character and plr.Character) and _G.Reach then
            ...
            for z,x in pairs(v.Character:GetChildren()) do
                if ((nearestPart.Position - v.Character.Torso.Position).Magnitude < _G.R) then
...
function shoot()
    ...
    if arc ~= nil and plr.Character and plr.Character:FindFirstChild("Humanoid") then
        local args = nil
        if _G.Silent == true then
            args = table(plr.Character.Torso.Position, (goal.Position + Vector3.new(math.random(-0.9,0.9), arc + math.random(-0.9,0.9), math.random(-0.9,0.9)) - plr.Character.HumanoidRootPart.Position + plr.Character.Humanoid.MoveDirection).Unit)
        elseif _G.Silent == false then
